...
Notebooks:
- `paper-stars.ipynb` fits the five Pleiades stars of the paper and combines their ages into an age for the cluster
- `new-star.ipynb` goes through everything needed for a star of your own: the mode table, the priors, the covariance matrix and the fit
...
pip install -e .
```
Run the notebooks from the root of the repo, because the star folders are read from `stars/`.

## Licence:
MIT, see `LICENSE`. It covers the code, the trained emulator, the mode tables and the calibration data.
